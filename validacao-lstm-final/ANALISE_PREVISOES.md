...
| **C** | Log-returns | Z-score | 0.672 | 0.630 | 0.620 | 0.700 | +0.205 | 0.660 | 22 | 0.035 ✅ |
...
## Por Que Classificação Direcional é Melhor que Regressão para Aplicações Financeiras

Baseado na análise dos 7 artigos revisados e nos resultados obtidos com xLSTM-TS e Transformer-TS, a **classificação direcional (subida/descida)** é mais adequada para aplicações financeiras reais que a **regressão de valor absoluto**.

### Evidência dos Artigos

**Artigos focados em trading/ganho financeiro** (Artigo 1, Artigo 4) usam **classificação binária** para gerar sinais de compra/venda diretos, avaliando com métricas financeiras (ganho de capital, retorno percentual, lucro por operação).

**Artigos focados em previsão técnica/acadêmica** (Artigo 2, Artigo 3, Artigo 7) usam **regressão** para prever valores exatos, avaliando com métricas de erro (MSE, RMSE, R²). Porém, o Artigo 2 mostra que o Ensemble teve melhor erro (MAPE=0.54) mas pior acerto de direção (70%) que MLP/NARX (80%).

### R² Não é Adequado para Aplicações Financeiras

R² mede precisão de valor absoluto, não ganho financeiro. Um modelo pode ter R² pior que a persistência mas ainda ganhar dinheiro se acertar a direção.

**Exemplo ilustrativo**:
- Modelo prevê 4.600 (compra) → real 4.505 → ganhou 5 pontos
- Persistência prevê 4.499 (venda) → real 4.505 → perdeu 5 pontos
- Persistência vence em R² (erro menor), mas modelo vence em dinheiro

### Métricas Financeiras Reais

Traders usam estas métricas, não R²:

| Métrica | O que mede | Valor bom |
|---------|------------|-----------|
| Return | Ganho total | > 20% |
| Sharpe Ratio | Retorno ajustado ao risco | > 2.0 |
| Maximum Drawdown | Maior queda | < 20% |
| Win Rate | Taxa de acerto | > 60% |
| Profit Factor | Razão lucro/perda | > 2.0 |

### Conclusão

Para aplicações financeiras reais, a **classificação direcional** é mais promissora porque:
1. O baseline de persistência em direção (~55-60%) é menos dominante que em valor (R²=0.9987)
2. Modelos podem superar o baseline em direção, mas não em valor absoluto
3. Métricas financeiras (win rate, return) são mais relevantes que métricas de erro (R², RMSE)

### Avaliação: Classificação Direta vs Derivada
Se aplicássemos classificação direta (treinar para prever subida/descida diretamente) em vez de derivar da regressão, as previsões seriam diferentes porque:

- Função de perda diferente: BCE em vez de MSE → otimização focada em acurácia de direção
- Distribuição temporal diferente: xLSTM-TS foca 94.4% no último timestep para minimizar erro de valor. Em classificação direta, poderia distribuir mais atenção por timesteps anteriores
- Sensibilidade a magnitude: Classificação direta poderia aprender que movimentos pequenos são mais ruidosos
- Resultado esperado: Acurácia poderia melhorar (atualmente 75.8% xLSTM, 75.6% Transformer) e a xAI mostraria importância temporal mais distribuída.

### Diferença entre modelos aplicados ao realizar previsão de classificação

- LSTM (18 features) → classificação direcional (já funcional)
- xLSTM-TS/Transformer-TS (1 feature) → classificação derivada da regressão (modelos originais já treinados)

Modelos de regressão de preço com features limitadas não conseguem classificação direcional eficaz, mas podem ter mérito em prever valores absolutos.

As arquiteturas xLSTM-TS e Transformer-TS (originalmente projetadas para regressão de preço com 1 feature) não são adequadas para classificação direcional, mesmo com as mesmas features do LSTM. O LSTM baseline funciona melhor porque tem arquitetura e hiperparâmetros otimizados especificamente para classificação.