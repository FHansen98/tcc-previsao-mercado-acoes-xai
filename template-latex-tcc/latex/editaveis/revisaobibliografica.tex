...
\hline
artigo6 & GBDT (Gradient Boosting Decision Trees) & Metodologia adequada para identificar regimes de mercado. É o método mais adequado para problemas de classificação em \textit{datasets} pequenos e desbalanceados (classes imbalanced). \\
\hline
...
A revisão bibliográfica demonstrou a evolução das técnicas de previsão no mercado financeiro, partindo de modelos clássicos de \textit{Machine Learning}, que frequentemente servem como \textit{benchmarks}, para arquiteturas mais sofisticadas de \textit{Deep Learning}, como as redes LSTM, e modelos de \textit{Ensemble}, notadamente o \textit{Gradient Boosting}. Foi observado que a abordagem do problema varia entre objetivos de Regressão (previsão de valor) e Classificação (previsão de movimento ou risco), com métricas de avaliação específicas para cada caso. A análise também destacou a importância do contexto dos dados, contrastando a maior estabilidade de mercados desenvolvidos com a volatilidade e as oportunidades de mercados emergentes, além de ressaltar o papel crucial da engenharia de \textit{features} (técnicas e macroeconômicas) para o sucesso dos modelos. Por fim, evidenciou-se que o poder preditivo por si só é insuficiente, consolidando a Inteligência Artificial Explicável (XAI), especialmente com ferramentas como o SHAP, como uma necessidade para auditar e conferir transparência às decisões dos modelos. Este trabalho se insere nesse contexto, buscando unir um modelo de previsão de alta performance com técnicas de XAI para gerar não apenas previsões, mas também explicações sobre o comportamento do mercado financeiro.
