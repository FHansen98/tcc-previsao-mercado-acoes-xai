...
Este capítulo apresenta os resultados dos experimentos realizados no período de treino (2000--2020), validação (2021--2022) e teste (2023--2024), sobre os dados diários do S\&P~500. Os resultados são apresentados, porém serão discutidos e interpretados no Capítulo~\ref{ch:discussao}.

\section{Ablation Study: Variantes de Pré-processamento da LSTM}
\label{sec:res_ablation}

O \textit{ablation study} avaliou seis variantes de pré-processamento aplicadas à LSTM para determinar a combinação de entrada e normalização com melhor desempenho direcional. As variantes diferem no tipo de entrada (\textit{Close} bruto, \textit{Close} DWT denoised ou Log-retornos DWT denoised) e no tipo de normalização (Z-score ou Min-Max), conforme definido na Metodologia.
...
Os modelos xLSTM-TS e Transformer-TS foram aplicados ao S\&P~500 com o mesmo pré-processamento da Variante~B adaptado (DWT causal + Min-Max fit-treino), recebendo como entrada apenas o \textit{Close} denoised com janela de 150 dias. Ambos foram treinados para regressão do preço absoluto com perda MSE e com hiperparâmetros idênticos (4 blocos, embedding 64, 2 cabeças, batch size 16, lr $10^{-4}$, \textit{early stopping} com paciência 40), permitindo comparação direta entre as duas arquiteturas. A direção prevista foi derivada em pós-processamento pela diferença entre previsões consecutivas.

As Tabelas~\ref{tab:xlstm_metricas} e~\ref{tab:transformer_metricas} apresentam as métricas de regressão (MAE, RMSE, RMSSE, MAPE e $R^2$) e as métricas direcionais derivadas (Acurácia, Recall, Precisão por classe e F1) para o conjunto de teste de cada modelo.
...
A Tabela~\ref{tab:comparativo} compara os três modelos pelas métricas direcionais comuns (Acurácia e F1), uma vez que a LSTM realiza classificação direta e os modelos SOTA realizam regressão com direção derivada, tornando as demais métricas não comparáveis entre si.
...
% A Figura~\ref{fig:predictions_comparison} exibe as previsões direcionais dos três modelos ao longo do período de teste para facilitar a comparação visual do comportamento de cada arquitetura.

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=\textwidth]{figuras/predictions_comparison.png}
%     \caption{Comparação das previsões direcionais dos três modelos no conjunto de teste (2023--2024).}
